...
#ifdef GRTRACK
  printf("label check %zu %zu %zu\n", f_ind, f_ind+n, mark_ind);
  if (mark_ind >= f_ind && mark_ind < f_ind + n) {
    printf("labeling %u\n", mark_ind);
    float initial = 1.0;
    dfsan_label i_label = dfsan_create_label("xmllint.c:831", initial);
    dfsan_set_label(i_label, &buf[mark_ind - f_ind], 
        sizeof(buf[mark_ind - f_ind]));
  }
#endif
...
static void parseAndPrintFile(char *filename, xmlParserCtxtPtr rectxt) {
...
#ifdef GRTRACK
              printf("label check %zu %zu %zu\n", f_ind, f_ind + res, mark_ind);
              if (mark_ind >= f_ind && mark_ind < f_ind + res) {
                printf("labeling %u\n", mark_ind);
                float initial = 1.0;
                dfsan_label i_label = dfsan_create_label("xmllint.c:2226", initial);
                dfsan_set_label(i_label, &chars[mark_ind - f_ind], 
                    sizeof(chars[mark_ind - f_ind]));
              }
#endif
                ...
                while ((res = myRead(f, chars, pushsize)) > 0) {
	...
	if (push) {
		...
		/* if (repeat) size = 1024; */
		/*res = fread(chars, 1, 4, f);*/
		res = myRead(f, chars, 4);
		if (res > 0) {
		    ctxt = xmlCreatePushParserCtxt(NULL, NULL,
		                chars, res, filename);
		    xmlCtxtUseOptions(ctxt, options);
				/*while ((res = fread(chars, 1, size, f)) > 0) {*/
		    while ((res = myRead(f, chars, size)) > 0) {
