...
xmlFileRead (void * context, char * buffer, int len) {
    ...
    long f_ind = ftell((FILE *) context);
    ret = fread(&buffer[0], 1,  len, (FILE *) context);

#ifdef GRTRACK
  printf("label check %zu %zu %zu\n", f_ind, f_ind + ret, mark_ind);
  if (mark_ind >= f_ind && mark_ind < f_ind + ret) {
    printf("labeling %u\n", mark_ind);
    float initial = 1.0;
    dfsan_label i_label = dfsan_create_label("xmllint.c:831", initial);
    dfsan_set_label(i_label, &buffer[mark_ind - f_ind], 
        sizeof(buffer[mark_ind - f_ind]));
  }
#endif
