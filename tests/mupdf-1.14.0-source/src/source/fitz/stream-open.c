...
#ifdef GRTRACK
  printf("label check %zu %zu %zu\n", stm->pos, stm->pos+n, mark_ind);
  if (mark_ind >= stm->pos && mark_ind < stm->pos + n) {
    printf("labeling %u\n", mark_ind);
    float initial = 1.0;
    dfsan_label i_label = dfsan_create_label("stream-open:107", initial);
    dfsan_set_label(i_label, &state->buffer[mark_ind - stm->pos], 
        sizeof(state->buffer[mark_ind - stm->pos]));
  }
#endif
