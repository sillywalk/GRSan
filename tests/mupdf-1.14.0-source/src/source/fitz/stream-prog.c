...
#ifdef GRTRACK
  printf("next-prog label check %zu %zu %zu\n", stm->pos, stm->pos+n, mark_ind);
  if (mark_ind >= stm->pos && mark_ind < stm->pos + n) {
    printf("labeling %u\n", mark_ind);
    float initial = 1.0;
    dfsan_label i_label = dfsan_create_label("stream-progg:56", initial);
    dfsan_set_label(i_label, &buf[mark_ind - stm->pos], 
        sizeof(buf[mark_ind - stm->pos]));
  }
#endif
