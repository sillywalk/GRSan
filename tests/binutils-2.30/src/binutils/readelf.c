...
#define IDX_IN_RANGE(N, START, SIZE)	({		\
	int retval = 0;					\
...
#define MARK_GRAD(N, A, S)	({						\
	float initial = 1.0;							\
	printf("[MARK_GRAD] labeling for byte index: %zu\n", mark_ind);		\
	dfsan_label i_label = dfsan_create_label((N), initial);			\
	dfsan_set_label(i_label, (A), (S));					\
	})
#endif
	  ...
	  const char *   reason)
{
  ...
  size_t pos = ftell(filedata->handle);
...
#ifdef GRTRACK
  if (IDX_IN_RANGE(mark_ind, pos, (nmemb * size))) {
    MARK_GRAD("readelf_input_file", &mvar[mark_ind - pos], (size_t) nmemb * size);
  }
#endif
...
#else
	  (argc, argv, "ADHINR:SVWacdeghi:k:lnp:rstuvw::x:z", options, NULL)) != EOF)
#endif
    {
      switch (c)
	{
...
#ifdef GRTRACK
  if (IDX_IN_RANGE(mark_ind, pos, number * ent_size)) {
    MARK_GRAD("readelf_input_file", &e_data[mark_ind - pos], (size_t) number * ent_size);
  }
#endif
...
process_symbol_table (Filedata * filedata)
{
  ...
  if (dynamic_info[DT_HASH]
	...
	hash_ent_size = 8;
      ...
      long b_pos = ftell(filedata->handle);
...
#ifdef GRTRACK
      if (IDX_IN_RANGE(mark_ind, b_pos, hash_ent_size)) {
        MARK_GRAD("readelf_input_file", &nb[mark_ind - b_pos], hash_ent_size);
      }
#endif

      long c_pos = ftell(filedata->handle);
...
#ifdef GRTRACK
      if (IDX_IN_RANGE(mark_ind, c_pos, hash_ent_size)) {
        MARK_GRAD("readelf_input_file", &nc[mark_ind - c_pos], hash_ent_size);
      }
#endif
  ...
  if (dynamic_info_DT_GNU_HASH
		 ...
		 SEEK_SET))
	{
      ...
      long pos = ftell(filedata->handle);
...
#ifdef GRTRACK
      if (IDX_IN_RANGE(mark_ind, pos, 16)) {
        MARK_GRAD("readelf_input_file", &nb[mark_ind - pos], 16);
      }
#endif
		 ...
		 SEEK_SET))
	{
      ...
      do
	{
    long m_pos = ftell(filedata->handle);
...
#ifdef GRTRACK
    if (IDX_IN_RANGE(mark_ind, m_pos, 4)) {
      MARK_GRAD("readelf_input_file", &nb[mark_ind - m_pos], 4);
    }
#endif
	  if (maxchain + 1 == 0)
	    goto no_gnu_hash;
...
#ifdef GRTRACK
  if (IDX_IN_RANGE(mark_ind, pos, EI_NIDENT)) {
    MARK_GRAD("readelf_input_file", &(filedata->file_header.e_ident)[mark_ind - pos],
        EI_NIDENT);
  }
#endif
  ...
  if (is_32bit_elf)
    {
      ...
      if (fread (ehdr32.e_type, sizeof (ehdr32) - EI_NIDENT, 1, filedata->handle) != 1)
        return FALSE;
#ifdef GRTRACK
      if (IDX_IN_RANGE(mark_ind, pos, sizeof (ehdr32) - EI_NIDENT)) {
        MARK_GRAD("readelf_input_file", &(ehdr32.e_type)[mark_ind - pos], sizeof (ehdr32) - EI_NIDENT);
      }
#endif
...
64 bit data type and so it cannot read 64 bit ELF files.\n"));
      ...
      if (fread (ehdr64.e_type, sizeof (ehdr64) - EI_NIDENT, 1, filedata->handle) != 1)
        return FALSE;
#ifdef GRTRACK
      if (IDX_IN_RANGE(mark_ind, pos, sizeof (ehdr64) - EI_NIDENT)) {
        MARK_GRAD("readelf_input_file", &(ehdr64.e_type)[mark_ind - pos],
            sizeof (ehdr64) - EI_NIDENT);
      }
#endif
...
#ifdef GRTRACK
      /* TODO if multiple bytes multiply by offset */
      if (IDX_IN_RANGE(mark_ind, pos, sizeof arch.arhdr)) {
        MARK_GRAD("readelf_input_file", &arch.arhdr, sizeof arch.arhdr);
      }
#endif
...
#ifdef GRTRACK
  /* TODO if multiple bytes multiply by offset */
  if (IDX_IN_RANGE(mark_ind, pos, SARMAG)) {
    MARK_GRAD("readelf_input_file", &armag, 1 * SARMAG);
  }
#endif
