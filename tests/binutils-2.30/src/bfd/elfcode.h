...
#ifdef MANUAL
  {
    dfsan_label lbls = dfsan_get_label(src->sh_size[0]);
    if (lbls) {
      struct dfsan_label_info * lbls_i = dfsan_get_label_info(lbls);
      printf("src->sh_size: %u %f %f %u", lbls, lbls_i->neg_dydx,
          lbls_i->pos_dydx, mark_ind);
    }

    dfsan_label lbld = dfsan_get_label(src->sh_size);
    if (lbld) {
      struct dfsan_label_info * lbld_i = dfsan_get_label_info(lbld);
      printf("dst->sh_size: %u %f %f %u", lbld, lbld_i->neg_dydx,
          lbld_i->pos_dydx, mark_ind);
    }
  }
#endif
