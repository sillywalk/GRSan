...
#ifdef MANUAL
  dfsan_label lbl = dfsan_get_label(v);
  dfsan_label al = dfsan_get_label(addr[0]);
  if (lbl != 0) {
    const struct dfsan_label_info* lbl_i = dfsan_get_label_info(lbl);
    const struct dfsan_label_info* al_i = dfsan_get_label_info(al);
    int asdf = lbl_i->neg_dydx + al_i->pos_dydx;
  }
#endif
