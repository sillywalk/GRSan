...
cache_bread_1 (struct bfd *abfd, void *buf, file_ptr nbytes)
{
...
#ifdef GRTRACK
        /*printf("label check %zu %zu %zu\n", f_ind, f_ind + nread, mark_ind);*/

        if (mark_ind >= f_ind && mark_ind < f_ind + nread) {
          /* mark input here */
          float initial = 1.0;
          /*printf("labeling buf %zu\n", mark_ind);*/

          dfsan_label i_label = dfsan_create_label("read", initial);
          dfsan_set_label(i_label, &buf[mark_ind - f_ind], sizeof(buf[mark_ind - f_ind]));
        }
#endif
  ...
  // check labels before and after
#ifdef MANUAL
  dfsan_label pre_lbls[nbytes];
  dfsan_label post_lbls[nbytes];

  for (int i=0; i<nbytes; i++) {
    pre_lbls[i] = dfsan_get_label(((char*)buf)[i]);
    if (pre_lbls[i] != 0) {
      printf("pre albel\n");
    }
  }
...
#ifdef MANUAL
  for (int i=0; i<nbytes; i++) {
    post_lbls[i] = dfsan_get_label(((char*)buf)[i]);
    if (post_lbls[i] != 0) {
      printf("post albel\n");
    }
  }
