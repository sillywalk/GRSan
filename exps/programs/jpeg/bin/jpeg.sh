...
if [ ! $# -eq 3 ]; then
  echo "Usage: jpeg.sh <mode> <mark byte index> <input file>  (mode is taint or gradient)";
  exit 1;
fi
...
TAINT_EXE="$SCRIPTPATH/djpeg_taint"
GRADIENT_EXE="$SCRIPTPATH/djpeg_grad"
BUG_EXE="$SCRIPTPATH/djpeg_bug"
