...
static u8 run_target(int timeout) {
  ...
  */
    ...
    if((total_execs %100000) == 0 && (total_execs != 0))
      printf("total execs %d edge coverage %d.\n", total_execs,count_non_255_bytes(virgin_bits));
...
int parse_array(char * str, int * array){
...
void gen_mutate_slow1(){
    ...
    for(iter=0 ;iter<503; iter=iter+1){
        ...
        u8 low_step = 0;
        int index;
	for(index=low_index; index<up_index; index=index+1){
            int cur_up_step = 0;
            int cur_low_step = 0;
            if(sign[index] == 1){
        ...
        int step;
	for(step=0;step<up_step;step=step+1){
            int mut_val;
            int index;
	    for(index=low_index; index<up_index; index=index+1){
	...
	for(step=0;step<low_step;step=step+1){
            int index;
	    for(index=low_index; index<up_index;index=index+1){
    ...
    int rand_loc = 0;
    int del_count;
    for(del_count=0; del_count < 4096;del_count= del_count+1){
        del_loc = loc[del_count];
        if ((len- del_loc) <= 2)
            continue;
        cut_len = choose_block_len(len-1-del_loc);
...
void gen_mutate(int iter_cnt, int file_len, int num_hot){ 
    int tmout_cnt = 0;
    //flip interesting locations within 10 iterations
    int iter;
    ...
    int step;
    int iidd;
    printf("hot byte num %d\n", num_hot);
    /*
    for(iidd = 0; iidd < num_hot;iidd=iidd+1){
        memcpy(out_buf1, out_buf, file_len);        
        memcpy(out_buf2, out_buf, file_len);        
        //printf("#######single byte searahcing %d\n", iidd);
        for(step=0; step < 256; step = step + 1){ 
            out_buf1[loc[iidd]] = step;
    ...
    for(iter=0 ;iter<(int)(num_hot/2); iter=iter+1){
        //printf("##########^^^^^^^^^^^^^^^^###################DEBUG  %d\n",iter);
        memcpy(out_buf1, out_buf, file_len);        
        memcpy(out_buf2, out_buf, file_len);        
        //find mutation range for every iteration
        int low_index = iter*2;
        int up_index = (iter+1)*2;
        u8 up_step = 0;
        u8 low_step = 0;
        int index;
	    ...
	    for(step=0;step<low_step;step=step+1){
            ...
            write_to_testcase(out_buf2, file_len);    
            int fault = run_target(exec_tmout); 
            if (fault != 0){
                //printf("execute test case failed\n");
                if(fault == FAULT_CRASH){
                    char* mut_fn = alloc_printf("%s/crash_%d_%06d", "./crashes",round_cnt, mut_cnt);
                    int mut_fd = open(mut_fn, O_WRONLY | O_CREAT | O_EXCL, 0600);
                    ck_write(mut_fd, out_buf2, file_len, mut_fn);
                    free(mut_fn);
                    close(mut_fd);
                    mut_cnt = mut_cnt + 1;
                }
    ...
    for(iter=0 ;iter<(int)(num_hot/4); iter=iter+1){
        printf("##########^^^^^^^^^^^^^^^^###################DEBUG  %d\n",iter);
        memcpy(out_buf1, out_buf, file_len);        
        memcpy(out_buf2, out_buf, file_len);        
        //find mutation range for every iteration
        int low_index = iter*3;
        int up_index = (iter+1)*3;
        u8 up_step = 0;
        u8 low_step = 0;
        int index;
    ...
    for(iter=0 ;iter<(int)(num_hot/8); iter=iter+1){
        printf("##########^^^^^^^^^^^^^^^^###################DEBUG  %d\n",iter);
        memcpy(out_buf1, out_buf, file_len);        
        memcpy(out_buf2, out_buf, file_len);        
        //find mutation range for every iteration
        int low_index = iter*4;
        int up_index = (iter+1)*4;
        u8 up_step = 0;
        u8 low_step = 0;
        int index;
    ...
    */
    ...
    for(iter=0 ;iter<iter_cnt; iter=iter+1){
        printf("##########^^^^^^^^^^^^^^^^###################DEBUG  %d\n",iter);
        memcpy(out_buf1, out_buf, file_len);        
        memcpy(out_buf2, out_buf, file_len);        
        //find mutation range for every iteration
...
void fuzz_lop(char * grad_file, int sock){
    ...
    copy_file("taint_info_p", grad_file);
    ...
    while ((nread = getline(&line, &llen, stream)) != -1) {    
        ...
        */
        ...
        char* c_iter_str = strtok(strtok(NULL,"|"),"\n");
        
        printf("$$$$$$#######DEBUGGGGGGGG  %s %s \n", fn, c_iter_str); 
        
        iter_cnt = atoi(c_iter_str);
        //char* fn = strtok(strtok(NULL,"|"),"\n");
        
        int num_hot = parse_array(loc_str,loc);
        ...
        memset(out_buf1,0,10000);
        memset(out_buf2,0,10000);
        memset(out_buf,0, 10000);
        memset(out_buf3,0, 20000);
        ck_read(fn_fd, out_buf, file_len, fn);
        
        //generate mutation
        if(stage_num == 1)
            gen_mutate(iter_cnt, file_len, num_hot);
        else
            gen_mutate_slow1();
        close(fn_fd);
    }
...
void main(int argc, char*argv[]){
    int opt;
    while ((opt = getopt(argc, argv, "+i:o:l:n:")) > 0)
